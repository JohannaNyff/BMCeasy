Fit_M <-function(ZF, Compound, fixedP=0){
      Koeff1<-NULL
      Koeff2<-NULL
      Koeff3<-NULL
      Warning<-NULL
    ...
    print(mergeData)
      Kurve1<-try( drm(Subset$response~Subset$dose, 
                       lowerl=c(0.5,  min(Subset$dose)), upperl=c(25, Inf), 
    ...
    Kurve2<-try(  drm(Subset$response~Subset$dose, 
                      lowerl=c(0.5, 15,  min(Subset$dose)), upperl=c(25, 80, Inf), 
    ...
    Kurve3<-try(   drm(Subset$response~Subset$dose,
                       lowerl=c(0.5, 120,  min(Subset$dose)), upperl=c(25, 300, Inf), 
                       fct=LL.4(fixed=c(NA,NA, 100,NA)) )  )
    if(mode(Kurve3)=="list"){
      Koeff3=c(Kurve3$fit$par[1], Kurve3$fit$par[2], 100, Kurve3$fit$par[3])  
    }
    Df3=4          
    try(print(mselect(Kurve1)[1]) )
    try(print(mselect(Kurve2)[1]) )
    try(print(mselect(Kurve3)[1]) ) 
    
    
      if(!is.null(Koeff1) & !is.null(Koeff2) & !is.null(Koeff3)){
    
            if(mselect(Kurve3)[1]>mselect(Kurve2)[1]){ #less negative logL is better
              print("Fit3 is better than Fit2")
              ...
              print("Fit2 is better than Fit3")
      ...
      if(FitNr!=1){   #if at least one of Fit2 or Fit3 worked
        ...
        print(p)
        print(length(which(abs(100-mergeData$response)>10)))
        if ((FitNr==3) & (p<0.5)){   
          print("Fit3 is better than Fit1") 
        }else if((FitNr==2) & (p<0.5) & length(which(abs(100-mergeData$response)>10)>2)){ #only possible if at least 3 concentrations <90%
          print("Fit2 is better than Fit1")           
        }else {
          # print("Fit1 is the winner")
          Kurve=Kurve1
          Koeff=signif(Koeff1,3)
          FitNr=1
          Df=Df1
